...
public class AVL<T extends Comparable<T>> implements AVLInterface<T> {
    ...
    @Override
    public synchronized void add(Task data) { //Jack
        if (data == null) {
            ...
            ChildType[] childTypes = new ChildType[worstCase]; 	// Jack ?
            ...
            if (childTypes[parentCount - 1] == ChildType.LEFT) {
                current.setLeft(new Node<T>(data)); // Jack
            } else {
                current.setRight(new Node<T>(data)); // Jack
            }
    ...
    private synchronized void updateFactorAndHeight(Node<T> target) {
        // Get children
    ...
    private synchronized void rebalance(Node<T> target, Node<T> parent, ChildType type) {
     ...
     *            <code>parent</code>.
     */
    private synchronized void rotateRight(Node<T> target, Node<T> parent, ChildType type) {
     ...
     *            <code>parent</code>.
     */
    private synchronized void rotateLeft(Node<T> target, Node<T> parent, ChildType type) {
     ...
     */
    private synchronized void preorder(List<T> existing, Node<T> current) {
     ...
     */
    private synchronized void postorder(List<T> existing, Node<T> current) {
        if (current.getLeft() != null) {
     ...
     */
    private synchronized void inorder(List<T> existing, Node<T> current) {
        if (current.getLeft() != null) {
